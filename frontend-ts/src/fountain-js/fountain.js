...
export class Fountain {
	...
	parse(script, getTokens) {
		...
		if (typeof script !== 'string')
			throw new Error(
				`Script should be \`string\`, input was \`${Object.prototype.toString.call(
					script
				)}\`.`
			)
		try {
			...
			return {
				title: title
					? this.inlineLex
							.reconstruct(title.text)
							.replace('<br />', ' ')
							.replace(/<(?:.|\n)*?>/g, '')
					: undefined,
				html: {
					title_page: this.tokens
	...
	to_html(token) {
		...
		switch (token.type) {
			...
			case 'credit':
				return '<p class="credit">' + token.text + '</p>'
			case 'author':
				return '<p class="authors">' + token.text + '</p>'
			case 'authors':
				return '<p class="authors">' + token.text + '</p>'
			case 'source':
				return '<p class="source">' + token.text + '</p>'
			case 'notes':
				return '<p class="notes">' + token.text + '</p>'
			case 'draft_date':
				return '<p class="draft-date">' + token.text + '</p>'
			case 'date':
				return '<p class="date">' + token.text + '</p>'
			case 'contact':
				return '<p class="contact">' + token.text + '</p>'
			case 'copyright':
				return '<p class="copyright">' + token.text + '</p>'
			case 'scene_heading':
				return (
					'<h3' +
			...
			case 'synopsis':
				return '<p class="synopsis">' + token.text + '</p>'
			case 'note':
				return '<!-- ' + token.text + ' -->'
			case 'boneyard_begin':
				return '<!-- '
			...
			case 'action':
				return '<p>' + token.text + '</p>'
			case 'centered':
				return '<p class="centered">' + token.text + '</p>'
			case 'lyrics':
				return '<p class="lyrics">' + token.text + '</p>'
			case 'page_break':
				return '<hr />'
			case 'line_break':
				return '<br />'
