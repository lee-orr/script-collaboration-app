...
export default function createHostFileList(adapter: NetworkAdapter<Message>, local: FileList): FileList {
    ...
    adapter.setListener((message) => {
        switch (message.type) {
            case "CreateFile":
                ...
                break;
            case "GetCurrentList":
                adapter.sendMessage({type: "CurrentListResult", listings: local.getCurrentList()})
                break;
            case "RenameFile":
